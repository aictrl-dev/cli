...
describe("session processor model stream idle timeout", () => {
  ...
  test("suspends while a local tool executes and resumes for provider events", async () => {
    await using tmp = await tmpdir({
      config: {
    ...
    try {
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          const session = await Session.create({ title: "Slow local tool fixture" })
          ...
          const stream = spyOn(LLM, "stream").mockResolvedValue({
            fullStream: (async function* () {
              ...
              yield { type: "tool-call", toolCallId: "call_1", toolName: "slow", input: {} }
              const output = await runSlowTool()
              yield { type: "tool-result", toolCallId: "call_1", toolName: "slow", input: {}, output }
              await new Promise(() => {})
            })(),
