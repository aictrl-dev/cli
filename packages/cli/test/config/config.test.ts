...
async function writeManagedSettings(settings: object, filename = "aictrl.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "aictrl.json") {
...
test("ignores legacy tui keys in aictrl config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://aictrl.ai/config.json",
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "aictrl.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .aictrl directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const aictrlDir = path.join(dir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })
      const agentDir = path.join(aictrlDir, "agent")
...
test("loads agents from .aictrl/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const aictrlDir = path.join(dir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })

      const agentsDir = path.join(aictrlDir, "agents")
...
test("loads commands from .aictrl/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const aictrlDir = path.join(dir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })

      const commandDir = path.join(aictrlDir, "command")
...
test("loads commands from .aictrl/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const aictrlDir = path.join(dir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })

      const commandsDir = path.join(aictrlDir, "commands")
...
test("does not try to install dependencies in read-only AICTRL_CONFIG_DIR", async () => {
  ...
  const prev = process.env.AICTRL_CONFIG_DIR
  process.env.AICTRL_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.AICTRL_CONFIG_DIR
    else process.env.AICTRL_CONFIG_DIR = prev
  }
})
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "aictrl.json"),
        JSON.stringify({ $schema: "https://aictrl.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "aictrl.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .aictrl config
      const projectDir = path.join(dir, "project")
      const aictrlDir = path.join(projectDir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })
      ...
      // Local .aictrl config with different plugins
      await Filesystem.write(
        path.join(aictrlDir, "aictrl.json"),
        JSON.stringify({
          $schema: "https://aictrl.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .aictrl config with some overlapping plugins
      await Filesystem.write(
        path.join(aictrlDir, "aictrl.json"),
        JSON.stringify({
          $schema: "https://aictrl.ai/config.json",
...
// Note: preload.ts sets AICTRL_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .aictrl config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .aictrl directory config enables it
      const aictrlDir = path.join(dir, ".aictrl")
      await fs.mkdir(aictrlDir, { recursive: true })
      await Filesystem.write(
        path.join(aictrlDir, "aictrl.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/aictrl")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/aictrl")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-aictrl@2.4.3")).toBe("oh-my-aictrl")
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-aictrl@2.4.3", "file:///project/.aictrl/plugin/oh-my-aictrl.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.aictrl/plugin/oh-my-aictrl.js")
  })
  ...
  test("local plugin directory overrides global aictrl.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const aictrlDir = path.join(projectDir, ".aictrl")
        const pluginDir = path.join(aictrlDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("AICTRL_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["AICTRL_DISABLE_PROJECT_CONFIG"]
    process.env["AICTRL_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["AICTRL_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["AICTRL_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .aictrl/ directories when flag is set", async () => {
    const originalEnv = process.env["AICTRL_DISABLE_PROJECT_CONFIG"]
    process.env["AICTRL_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .aictrl directory with a command
          const aictrlDir = path.join(dir, ".aictrl", "command")
          await fs.mkdir(aictrlDir, { recursive: true })
          await Filesystem.write(path.join(aictrlDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .aictrl should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["AICTRL_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AICTRL_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["AICTRL_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["AICTRL_CONFIG_DIR"]
      } else {
        process.env["AICTRL_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("AICTRL_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["AICTRL_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AICTRL_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["AICTRL_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["AICTRL_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from AICTRL_CONFIG_DIR, not project
...
describe("AICTRL_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in AICTRL_CONFIG_CONTENT", async () => {
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["AICTRL_CONFIG_CONTENT"] = originalEnv
      } else {
  ...
  test("substitutes {file:} tokens in AICTRL_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AICTRL_CONFIG_CONTENT"]
