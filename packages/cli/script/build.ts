...
const modelsUrl = process.env.AICTRL_MODELS_URL || "https://models.dev"
// Fetch and generate models.dev snapshot
let modelsData = process.env.MODELS_DEV_API_JSON
  ? await Bun.file(process.env.MODELS_DEV_API_JSON).text()
  : await fetch(`${modelsUrl}/api.json`).then((x) => x.text())

if (liteFlag) {
  const allModels = JSON.parse(modelsData)
  const liteProviders = ["anthropic", "openai", "google", "google-vertex", "openrouter", "zai", "aictrl"]
  const filtered: Record<string, any> = {}
  for (const key of liteProviders) {
    if (allModels[key]) filtered[key] = allModels[key]
  }
  modelsData = JSON.stringify(filtered)
}
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    define: {
      ...
      AICTRL_CHANNEL: `'${Script.channel}'`,
      AICTRL_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
      AICTRL_HEADLESS: liteFlag ? "'true'" : "'false'",
    },
  })
