...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.AICTRL_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no AICTRL_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.AICTRL_CONFIG_DIR, Flag.AICTRL_CONFIG_DIR).catch(() => [])
}
